...
const getRecoders = async (force: boolean = false) => {
    ...
    try {
        ...
        var data: AxiosResponse = await api.post<any>('exchange/order-history')
        records.value = data.data.data
        userSession.setlastOrders(records.value)
    ...
    <div class="card mb-4">
        <div class="card-header custom-h">
            <h4 class="card-title h2  mb-0 ">
                <Vue3Lottie class="w-h-icon" :animationData="historyIcon" :height="40" :width="40" /> Last Orders
            </h4>
        </div>
        <div class="card-body p-0" v-if="!isLoading">
            ...
            <div class="table-responsive" v-if="records?.length">
                <table class="table table-centered border table-nowrap mb-0"
                    ...
                    <!-- end thead -->
                    <tbody>
                        <tr v-for="(row, index)  of records">
                            <td> {{                                                  dateFormat(row.created_at, 'mmm d, yyyy HH:MM:ss')                                                 }}
                            </td>
                            <td>
                                <CommaNum :num="row.base_amount"></CommaNum>
                                {{                       dex().tokenOf(row.pair.base_asset).token_name                       }}
                            </td>
                            <td>
                                <CommaNum :num="row.quote_amount"></CommaNum>
                                {{                         dex().tokenOf(row.pair.quote_asset).token_name                       }}
                            </td>
                            <td>
                                <b-badge :variant="row.type == 'SELL' ? 'danger' : 'success'">
                                    {{                                                  row.type                                                  }}
                                </b-badge>
                            </td>
